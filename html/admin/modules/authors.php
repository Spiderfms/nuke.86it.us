...
if (($row['radminsuper'] == 1) && ($row['name'] == 'God')) {
	...
	function displayadmins() {
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			$a_aid = substr((string) $a_aid, 0,25);
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			$title = preg_replace('#_#m', " ", (string) $row['title']);
	...
	function updateadmin($chng_aid, $chng_name, $chng_email, $chng_url, $chng_radminsuper, $chng_pwd, $chng_pwd2, $chng_admlanguage, $adm_aid, $auth_modules) {
		...
		if (!empty($chng_pwd2)) {
			...
			if ($chng_radminsuper == 1) {
				...
				$db->sql_query("UPDATE " . $prefix . "_authors SET aid='$chng_aid', 
				                                               email='$chng_email', 
															       url='$chng_url', 
												   radminsuper='$chng_radminsuper', 
												                   pwd='$chng_pwd', 
												   admlanguage='$chng_admlanguage' WHERE name='$chng_name' AND aid='$adm_aid'");
