		...
		switch ($mode)
		{
			case 'overview':
				...
				if ($submit)
				{
					...
					$update_postcount = ($user_row['user_posts'] != $data['user_posts']) ? intval($data['user_posts']) : false;
...
//-- fin mod : Genders --------------------------------------------------------
			...
			case 'flag':
				...
				while ($row = $db->sql_fetchrow($result))
				{
					$selected = ($user_row['user_flag'] && $row['flag_code'] == $user_row['user_flag']) ? ' selected="selected"' : '';
